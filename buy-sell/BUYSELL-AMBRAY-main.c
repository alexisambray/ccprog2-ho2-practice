...
/*
   ...
   Note that your BUYSELL-LASTNAME.c and MYHEADER-LASTNAME.h solution will be
   black box tested using another main() function which is different the one
   this file.
*/
int main() {
  struct sellerTag SELLERS[MAX_SELLERS];  // MAX_SELLERS is defined in buysell.h
  struct productTag
      PRODUCTS[MAX_PRODUCTS];  // MAX_PRODUCTS is defined buysell.h
